...
val scn = scenario("20240124205133197")
.exec(http("request_GET").get("/health-check").headers(headers_0))
.pause(1)
.exec(http("request_GET").get("/health-check").headers(headers_0))
.pause(1)
this.setUp(scn.injectOpen(atOnceUsers(10))).protocols(httpProtocol)}}
